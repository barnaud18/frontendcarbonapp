...
# Configure SQLite database for local development (no PostgreSQL required)
...
def propriedades():
    try:
        ...
        todas_propriedades = Propriedade.query.all()
        
        return render_template('propriedades.html', propriedades=todas_propriedades)
    
    except Exception as e:
        app.logger.error(f"Error retrieving properties: {str(e)}")
        flash(f"Erro ao buscar propriedades: {str(e)}", 'error')
        return redirect(url_for('index'))
